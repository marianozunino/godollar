...
func testRatesExists(t *testing.T) {
	...
	e, err := RateExists(ctx, tx, o.Date)
	if err != nil {
...
func testRatesFind(t *testing.T) {
	...
	rateFound, err := FindRate(ctx, tx, o.Date)
	if err != nil {
		t.Error(err)
	}
...
var (
	rateDBTypes = map[string]string{`Date`: `TIMESTAMP`, `BuyPrice`: `FLOAT`, `SellPrice`: `FLOAT`, `EbrouBuyPrice`: `FLOAT`, `EbrouSellPrice`: `FLOAT`}
	_           = bytes.MinRead
)
...
func testRatesUpsert(t *testing.T) {
	...
	if err = o.Upsert(ctx, tx, false, nil, boil.Infer(), boil.Infer()); err != nil {
		t.Errorf("Unable to upsert Rate: %s", err)
	}
	...
	if err = randomize.Struct(seed, &o, rateDBTypes, false, ratePrimaryKeyColumns...); err != nil {
		t.Errorf("Unable to randomize Rate struct: %s", err)
	}
